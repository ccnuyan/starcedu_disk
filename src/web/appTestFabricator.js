...
let rawIndexHTML = fs.readFileSync(path.join(__dirname, './appTest.html'), 'utf-8');
...
styles.push(`<link href="${assets.app_embed.css}" rel="stylesheet"/>`);
...
scripts.push(`<script src="${assets.app_embed.js}"></script>`);
scripts.push(`<script src="${assets.app_test.js}"></script>`);
