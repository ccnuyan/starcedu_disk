...
describe('crud_files', () => {
  before(async function init() {
    ...
    await pool.query('select * from starcedu_disk.register($1, $2)', [
      params.username,
      params.password,
    ]).then((res) => {
      userinfo = res.rows[0];
  ...
  describe('create a new file', () => {
    before(async () => {
      return pool.query('select * from starcedu_disk.create_file($1, $2)', [
        userinfo.id,
        file.filename,
      ]).then((res) => {
        fileInfo = res.rows[0];
  ...
  describe('update created file title', () => {
    before(async () => {
      return pool.query('select * from starcedu_disk.update_file($1, $2, $3)', [
        userinfo.id,
        fileInfo.id,
        `${file.filename}new`,
      ]).then((res) => {
  ...
  describe('update created file status', () => {
    before(async () => {
      return pool.query('select * from starcedu_disk.set_file_uploaded($1, $2, $3, $4, $5)', [
        userinfo.id,
        fileInfo.id,
        null,
        `${file.snapshot}new`,
  ...
  describe('delete created file', () => {
    before(async () => {
      return pool.query('select * from starcedu_disk.delete_file($1, $2)', [
        userinfo.id,
        fileInfo.id,
      ]).then((res) => {
  ...
  describe('created file can not ne required now', () => {
    before(async () => {
      return pool.query('select * from starcedu_disk.require_file($1)', [fileInfo.id]).then((res) => {
        requiredFileInfo = res.rows[0];
        return res;
      });
    });
